...
public class PrintXmxXms {
	...
	/**
	 * We're reusing this method in HeapSizingApplication, therefore this method was
	 * extracted
	 * to avoid repetition.
	 */
	static void logMemory(Logger logger) {
		...
		for (MemoryPoolMXBean mp : ManagementFactory.getMemoryPoolMXBeans()) {
			logger.log(Level.INFO, "Pool: {0} (type {1}) = {2}",
					new Object[] { mp.getName(), mp.getType(), mp.getUsage().getMax() / mb });
		}
	}
}