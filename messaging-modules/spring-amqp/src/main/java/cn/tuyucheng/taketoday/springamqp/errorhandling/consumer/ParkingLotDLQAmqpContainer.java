...
import cn.tuyucheng.taketoday.springamqp.errorhandling.configuration.DLXParkingLotAmqpConfiguration;
...
public class ParkingLotDLQAmqpContainer {
	...
	public void processFailedMessagesRetryWithParkingLot(Message failedMessage) {
		Integer retriesCnt = (Integer) failedMessage.getMessageProperties().getHeaders().get(MessagesConsumer.HEADER_X_RETRIES_COUNT);
		if (retriesCnt == null)
			retriesCnt = 1;
		if (retriesCnt > MAX_RETRIES_COUNT) {
			log.info("Sending message to the parking lot queue");
			rabbitTemplate.send(DLXParkingLotAmqpConfiguration.EXCHANGE_PARKING_LOT, failedMessage.getMessageProperties().getReceivedRoutingKey(), failedMessage);
			return;
		}
		log.info("Retrying message for the {} time", retriesCnt);
		failedMessage.getMessageProperties().getHeaders().put(MessagesConsumer.HEADER_X_RETRIES_COUNT, ++retriesCnt);
