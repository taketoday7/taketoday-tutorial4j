...
if [ "$1" = "native" ]; then
    ...
    docker build -f $SCRIPTPATH/src/main/docker/Dockerfile.native -t quarkus-project:1.0.0 $SCRIPTPATH/.
    ...
    docker build -f $SCRIPTPATH/src/main/docker/Dockerfile.jvm -t quarkus-project:1.0.0 $SCRIPTPATH/.
fi