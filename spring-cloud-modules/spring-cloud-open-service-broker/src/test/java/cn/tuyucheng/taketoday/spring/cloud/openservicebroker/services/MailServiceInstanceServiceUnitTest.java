...
class MailServiceInstanceServiceUnitTest {
   ...
   @Test
   void givenServiceInstanceDoesNotExist_whenCreateServiceInstance_thenProvisionNewService() {
   ...
   @Test
   void givenServiceInstanceExists_whenCreateServiceInstance_thenExistingServiceInstanceIsRetrieved() {
   ...
   @Test
   void givenServiceInstanceDoesNotExist_whenDeleteServiceInstance_thenException() {
      // given service instance does not exist
      when(mailService.serviceInstanceExists(MAIL_SERVICE_INSTANCE_ID)).thenReturn(Mono.just(false));
      ...
      StepVerifier.create(mailServiceInstanceService.deleteServiceInstance(request))
            .expectErrorMatches(ServiceInstanceDoesNotExistException.class::isInstance)
            .verify();
   }

   @Test
   void givenServiceInstanceExists_whenDeleteServiceInstance_thenSuccess() {
      // given service instance exists
      when(mailService.serviceInstanceExists(MAIL_SERVICE_INSTANCE_ID)).thenReturn(Mono.just(true));
   ...
   @Test
   void givenServiceInstanceDoesNotExist_whenGetServiceInstance_thenException() {
      // given service instance does not exist
   ...
   @Test
   void givenServiceInstanceExists_whenGetServiceInstance_thenExistingServiceInstanceIsRetrieved() {
      // given service instance exists
      MailServiceInstance serviceInstance = new MailServiceInstance(MAIL_SERVICE_INSTANCE_ID, MAIL_SERVICE_DEFINITION_ID,
            MAIL_SERVICE_PLAN_ID, MAIL_DASHBOARD_URL);
      when(mailService.getServiceInstance(MAIL_SERVICE_INSTANCE_ID)).thenReturn(Mono.just(serviceInstance));
