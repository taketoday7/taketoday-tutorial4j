...
public class ExploreSpring5URLPatternUsingRouterFunctions {
   ...
   private RouterFunction<ServerResponse> routingFunction() {
      ...
      return route(GET("/t?st"), serverRequest -> ok().body(fromValue("Path /t?st is accessed"))).andRoute(GET("/test/{*id}"), serverRequest -> ok().body(fromValue(serverRequest.pathVariable("id"))))
            ...
            .andRoute(GET("/{var1}_{var2}"), serverRequest -> ok().body(fromValue(STR."\{serverRequest.pathVariable("var1")} , \{serverRequest.pathVariable("var2")}")))
            .andRoute(GET("/{tuyucheng:[a-z]+}"), serverRequest -> ok().body(fromValue(STR."/{tuyucheng:[a-z]+} was accessed and tuyucheng=\{serverRequest.pathVariable("tuyucheng")}")))
