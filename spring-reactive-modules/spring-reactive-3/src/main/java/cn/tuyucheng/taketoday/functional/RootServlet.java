...
public class RootServlet extends ServletHttpHandlerAdapter {
   ...
   private static RouterFunction<?> routingFunction() {
      ...
      return route(GET("/test"), _ -> ok().body(fromValue("helloworld"))).andRoute(POST("/login"), serverRequest -> serverRequest.body(toFormData())
                  .map(MultiValueMap::toSingleValueMap)
                  .map(formData -> {
                     System.out.println(STR."form data: \{formData.toString()}");
            ...
            .andRoute(POST("/upload"), serverRequest -> serverRequest.body(toDataBuffers())
                  ...
                  .map(dataBuffers -> {
                     ...
                     dataBuffers.forEach(d -> atomicLong.addAndGet(d.asByteBuffer()
                           .array().length));
                     System.out.println(STR."data length:\{atomicLong.get()}");
                     return ok().body(fromValue(atomicLong.toString()))
                           .block();
                  }))
            .and(RouterFunctions.resources("/files/**", new ClassPathResource("files/")))
            .andNest(path("/actor"), route(GET("/"), _ -> ok().body(Flux.fromIterable(actors), Actor.class)).andRoute(POST("/"), serverRequest -> serverRequest.bodyToMono(Actor.class)
                  .doOnNext(actors::add)
                  .then(ok().build())))
            .filter((request, next) -> {
               System.out.println(STR."Before handler invocation: \{request.path()}");
