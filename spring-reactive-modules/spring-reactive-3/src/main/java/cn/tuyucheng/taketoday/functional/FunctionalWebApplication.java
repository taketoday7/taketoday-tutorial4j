...
public class FunctionalWebApplication {
   ...
   private RouterFunction<ServerResponse> routingFunction() {
      ...
      RouterFunction<ServerResponse> restfulRouter = route(GET("/actor"), _ -> ok().body(Flux.fromIterable(actors), Actor.class)).andRoute(POST("/actor"), serverRequest -> serverRequest.bodyToMono(Actor.class)
            .doOnNext(actors::add)
            .then(ok().build()));

      return route(GET("/test"), _ -> ok().body(fromValue("helloworld"))).andRoute(POST("/login"), formHandler::handleLogin)
