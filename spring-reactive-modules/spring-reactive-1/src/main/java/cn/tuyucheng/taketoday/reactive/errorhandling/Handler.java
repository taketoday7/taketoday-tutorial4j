...
public class Handler {
   ...
   public Mono<ServerResponse> handleWithErrorResumeAndDynamicFallback(ServerRequest request) {
      return sayHello(request)
            .flatMap(s -> ServerResponse.ok()
                  .contentType(MediaType.TEXT_PLAIN)
                  .bodyValue(s))
            .onErrorResume(e -> (Mono.just(STR."Hi, I looked around for your name but found: \{e.getMessage()}"))
                  .flatMap(s -> ServerResponse.ok()
                        .contentType(MediaType.TEXT_PLAIN)
                        .bodyValue(s)));
   }
   ...
   public Mono<ServerResponse> handleWithErrorResumeAndFallbackMethod(ServerRequest request) {
      return sayHello(request)
            .flatMap(s -> ServerResponse.ok()
                  .contentType(MediaType.TEXT_PLAIN)
                  .bodyValue(s))
            .onErrorResume(_ -> sayHelloFallback()
                  .flatMap(s -> ServerResponse.ok()
                        .contentType(MediaType.TEXT_PLAIN)
                        .bodyValue(s)));
   }
   ...
   private Mono<String> sayHello(ServerRequest request) {
      try {
         return Mono.just(STR."Hello, \{request.queryParam("name").get()}");
      } catch (Exception e) {
         return Mono.error(e);
      }
   }
