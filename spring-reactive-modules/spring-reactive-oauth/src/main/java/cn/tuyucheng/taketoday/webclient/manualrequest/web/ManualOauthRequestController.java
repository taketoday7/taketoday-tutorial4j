...
public class ManualOauthRequestController {
   ...
   private static final Logger logger = LoggerFactory.getLogger(ManualOauthRequestController.class);
   ...
   public Mono<String> obtainSecuredResource() {
      ...
      Mono<String> resource = client.post()
            .uri(tokenUri)
            .header(HttpHeaders.AUTHORIZATION, STR."Basic \{Base64.getEncoder()
                  .encodeToString((STR."\{clientId}:\{clientSecret}").getBytes())}")
      ...
      return resource.map(res -> STR."Retrieved the resource using a manual approach: \{res}");
   }
}