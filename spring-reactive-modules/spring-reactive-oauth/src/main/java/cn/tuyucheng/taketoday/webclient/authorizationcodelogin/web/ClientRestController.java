...
public class ClientRestController {
   ...
   Mono<String> useOauthWithAuthCode() {
      ...
      return retrievedResource.map(string -> STR."We retrieved the following resource using Oauth: \{string}");
   }
   ...
   Mono<String> useOauthWithAuthCodeAndAnnotation(@RegisteredOAuth2AuthorizedClient("bael") OAuth2AuthorizedClient authorizedClient) {
      Mono<String> retrievedResource = otherWebClient.get()
            ...
            .retrieve()
            .bodyToMono(String.class);
      return retrievedResource.map(string -> STR."We retrieved the following resource using Oauth: \{string}. Principal associated: \{authorizedClient.getPrincipalName()}. Token will expire at: \{authorizedClient.getAccessToken()
            .getExpiresAt()}");
   }
