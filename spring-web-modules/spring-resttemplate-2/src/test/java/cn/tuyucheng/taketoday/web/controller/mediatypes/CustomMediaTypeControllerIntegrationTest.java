package cn.tuyucheng.taketoday.web.controller.mediatypes;
...
public class CustomMediaTypeControllerIntegrationTest {
    ...
    public void givenServiceUrl_whenGetWithProperAcceptHeaderFirstAPIVersion_thenReturn200() throws Exception {
        mockMvc.perform(get("/public/api/items/1").accept("application/vnd.tuyucheng.api.v1+json")).andExpect(status().isOk());
    }

    @Test
    public void givenServiceUrl_whenGetWithProperAcceptHeaderSecondVersion_thenReturn200() throws Exception {
        mockMvc.perform(get("/public/api/items/2").accept("application/vnd.tuyucheng.api.v2+json")).andExpect(status().isOk());
    }
}